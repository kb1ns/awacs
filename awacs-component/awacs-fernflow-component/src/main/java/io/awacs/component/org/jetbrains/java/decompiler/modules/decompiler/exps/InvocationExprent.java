...
import io.awacs.component.org.jetbrains.java.decompiler.modules.decompiler.ClasspathHelper;
...
import io.awacs.component.org.jetbrains.java.decompiler.struct.gen.MethodDescriptor;
...
public class InvocationExprent extends Exprent {
    ...
    if (functype == TYP_INIT) {
      ...
      if (newNode != null) {  // own class
        if (newNode.getWrapper() != null) {
          ...
          if (newNode.type == ClassesProcessor.ClassNode.CLASS_MEMBER && (newNode.access & CodeConstants.ACC_STATIC) == 0) { // non-static member class
  ...
  public static Exprent unboxIfNeeded(Exprent param) {
    ...
    if (param.type == EXPRENT_INVOCATION && ((InvocationExprent)param).isBoxingCall()) {
  ...
  private boolean isBoxingCall() {
    if (isStatic && "valueOf".equals(name) && lstParameters.size() == 1) {
      ...
      if (lstParameters.get(0).type == EXPRENT_CONST) {
